...
if __name__ == "__main__":
    ...
    parser.add_argument("input_file", help="path to input file", default="input1.txt")
    parser.add_argument("-w", "--window_size", help="sliding window size", default=1)
    args = parser.parse_args()

    with open(args.input_file, "r") as file:
        depths = [int(line) for line in file.readlines()]

    output = solve(depths, window_size=int(args.window_size))
    print(output)
